...
(function($) {
    jQuery.fn.imageMaps = function(setting) {
		...
		$container.each(function(){
			var container = $(this);//整个大容器div#imgMap
			var $images = container.find('img[ref=imageMaps]');//操作的图片
			...
			$images.each(function(){
				...
				_img_conrainer.append('<div class="button-conrainer" style="position:fixed;right: 0;top:209px;z-index: 999;width: 70px;padding:0;"><input class="add-area-bt" type="button" value="添加热区" style="height:26px;width:70px;margin-bottom:1px;" /><input class="save-area-bt" type="button"  value="保存" style="height:26px;width:70px;margin-bottom:1px;"/></div>').append('<div class="link-conrainer" style="position:fixed;right: 0;top:160px;z-index: 888;background: rgba(0,0,0,0.5);width: 400px;min-height:120px;color:#fff;"><p style="margin:10px 0;"><span style="padding:0 4px;">序号</span><span style="padding:0 20px;">名称</span><span style="padding:0 40px;">链接</span></p></div>').append($.browser.msie ? $('<div class="position-conrainer" style="position:absolute"></div>').css({
					background:'#fff',
					opacity:0
				...
				_img_conrainer.find('.position-conrainer').css({//div.position-conrainer是一个定位的容器
				...
				var map_name = $(this).attr('usemap').replace('#','');//值为'Map'
				if(map_name !=''){//根据图片usemap熟悉对应的<map>里的<area>来初始化 div.link-conrainer和div.position-conrainer里的内容
					var index = 1;
					var _link_conrainer = _img_conrainer.find('.link-conrainer');
					var _position_conrainer = _img_conrainer.find('.position-conrainer');
					var image_param = $(this).attr('name') == '' ? '' : '['+ $(this).attr('name') + ']';//值为test
					container.find('map[name='+map_name+']').find('area[shape=rect]').each(function(){
						var coords = $(this).attr('coords');
						var areaName=$(this).attr('data-area-name');//cny_add 热区名称
						_link_conrainer.append('<p ref="'+index+'" class="map-link" style="margin:10px 0;"><span class="link-number-text" style="padding:0 14px;color:#fff;"> '+index+'</span><input type="text" style="display: inline-block;width:80px;" value="'+areaName+'"/> <input type="text" style="display: inline-block;width:180px;" name="link'+image_param+'[]" value="'+$(this).attr('href')+'" /><input type="hidden" class="rect-value" name="rect'+image_param+'[]" value="'+coords+'" /></p>');
						coords = coords.split(',');
						_position_conrainer.append('<div ref="'+index+'" class="map-position" style="left:'+coords[0]+'px;top:'+coords[1]+'px;width:'+(coords[2]-coords[0])+'px;height:'+(coords[3]-coords[1])+'px;"><div class="map-position-bg"></div><span class="link-number-text">热区 '+index+'</span><span class="delete">X</span><span class="resize"></span></div>');
						index++;
					});
				}
			});
		...
		$('.button-conrainer  input[type=button].add-area-bt').click(function(){
			...
			image = (image == '' ? '' : '['+ image + ']');
			_link_conrainer.append('<p ref="'+index+'" class="map-link"  style="margin:10px 0;"><span class="link-number-text" style="padding:0 14px;color:#fff;"> '+index+'</span><input type="text" style="display: inline-block;width:80px;" value="新热区"/> <input type="text" style="display: inline-block;width:180px;" name="link'+image+'[]" value="" /><input type="hidden" class="rect-value" name="rect'+image+'[]" value="10,10,100,40" /></p>');
