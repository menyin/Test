 ...
 * @param {string|date} dEndDate 被减数
 * @param {number} dEndDate 定时毫秒数
 ...
 */
 function interval(dEndDate,times,callback) {
    var timer = setInterval(function () {
        var dNowDate=new Date();
        console.log(dNowDate.toLocaleString());
        var oSub=getSubTimes(dNowDate, dEndDate);
        if (oSub.day==0&&oSub.hour==0&&oSub.min==0&&oSub.second==0) {
            clearInterval(timer);
        }
        if (callback) {
            callback(oSub);
